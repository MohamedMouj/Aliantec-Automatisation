...
def index(request):
    ...
    if request.method == 'POST' and request.FILES.get('input_excel'):
        ...
        try:
            ...
            request.session['extraction_vin_download'] = str(output_path)
            request.session['extraction_vin_download_name'] = output_name
...
def download_file(request):
    file_path = request.session.get('extraction_vin_download')
    file_name = request.session.get('extraction_vin_download_name', 'output.xlsx')
    ...
    # Clean up session keys after download is served
    request.session.pop('extraction_vin_download', None)
    request.session.pop('extraction_vin_download_name', None)
