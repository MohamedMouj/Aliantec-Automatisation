...
@log_execution('Extraction SM', action='Excel Extraction', project_name=lambda request: '', filename=lambda request: request.FILES.get('input_excel').name if request.FILES.get('input_excel') else '')
def index(request):
